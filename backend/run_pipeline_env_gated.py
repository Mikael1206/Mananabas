...
pipeline helpers (transcription, captions, reframe, render) without needing a
valid `LLM_PROVIDER` / `OPENAI_API_KEY` etc. configured first.

For the full end-to-end runner (with env validation at import time), see
`run_pipeline.py`.
"""
...
# NOTE: no env validation here at import time. main() does that.
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print("Usage: python run_pipeline_env_gated.py <youtube_url>", file=sys.stderr)
        sys.exit(1)
